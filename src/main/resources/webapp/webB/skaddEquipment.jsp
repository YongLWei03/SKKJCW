...
<%-- 			<input type="hidden" id="hname" value="${user.name }"> --%>
	...
	function insert(){
		...
		$.ajax({    
		    url:'${path}/webSkEquipment/insetEquipment',
		    data:{    
		    ...
		    success:function(data) {
		    	if(data=="true"){
		    		location.href="${path}/webSkEquipment/selectEquipment?="+currentPage;
		    	}else if(data == "false"){
		    		layer.msg('添加失败');
                    layer.closeAll('loading');
		    	}else if(data == "bdsb"){
	...
	function remo(value){
		...
		location.href="${path}/webSkEquipment/selectEquipment?currentPage="+currentPage;
	}
	...
	function upda(eqId){
		...
		$.ajax({
		    url:'${path}/webSkEquipment/updateByeqId',
		    data:{
                'deviceName' : deviceName,
                'headOfEquipment' : headOfEquipment,
                'phone':phone,
