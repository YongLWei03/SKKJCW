				...
				<div style="width: 1200px; margin: auto;">
					...
					<table class="table table-bordered table-hover table-striped"
						...
						<tbody>
							...
							<c:forEach var="buser" items="${userRiglist}" varStatus="s">
								<tr align="center" style="height: 35px;">
									<td>${s.index+1}</td>
									<td>${buser.name}</td>
									<td>
									...
									<td>${buser.region}</td>
										<td>
											<c:forEach var="userRegionLists" items="${userRegionLists}" varStatus="s">
													<c:if test="${userRegionLists.userId == buser.userId }">
														${userRegionLists.substationName}
													</c:if>
											</c:forEach>
										</td>
										<td>${buser.remarks}</td>
									<td>
										<a href="javascript:void(0);" onclick="sava(${buser.userId})">授权</a>
...
// 		var keyword = $("#keyword").val();
		location.href="${path}/webUser/userRights?currentPage="+p;
	}
	...
	function sava(userId){
		location.href="${path}/webUser/serUserRights?userId="+userId+"&flog=2";
	}
