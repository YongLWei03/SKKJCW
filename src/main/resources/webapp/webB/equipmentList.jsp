...
<div id="middle">
    ...
    <div class="right" id="mainFrame">
        ...
        <div class="right_cont">
            ...
            <input type="hidden" value="${currentPage}" id= "currentPages">
            <div style="width: 1200px; margin: auto;">
                    ...
                    <tr>
                        <c:if test="${user.type == 1 || user.type == '1' }">
                            ...
                            <td width="10%">
                                <c:if test="${user.type == 1 || user.type == '1'}">
                                    <input
                                            type="button" id = "inuser"  value="批量导入" onclick="inserAll();"class="btn btn-primary"
                                            style="width: 94px; height: 28px; font-family: 微软雅黑;" />
                                </c:if>
                            </td>
                        </c:if>
                        <%--<td width="5%">--%>
                            <%--<select id="type" style="width: 94px; height: 28px;">--%>
                                <%--<c:if test="${type == 0 || type == '0' || type == '' || type == null}">--%>
                                ...
                                <%--<c:if test="${type == 3 || type == '3' }">--%>
                                    ...
                                    <%--<option value="1">正常</option>--%>
                                    <%--<option value="2">警告</option>--%>
                                    <%--<option value="3"  selected="selected">异常</option>--%>
                                <%--</c:if>--%>
                            <%--</select>--%>
                        <%--</td>--%>
                        <%--<td>--%>
                            <%--<input type="button"  value="查询" onclick="goback(1);"class="btn btn-primary"--%>
                                   <%--style="width: 94px; height: 28px; font-family: 微软雅黑;" />--%>
                        <%--</td>--%>
                        <td></td>
                    </tr>
                ...
                <table class="table table-bordered table-hover table-striped"
                    ...
                    <c:forEach var="equilist" items="${equilist}" varStatus="s">
                        ...
                        <tr align="center" style="height: 35px;">
                            <td>${s.index+1}</td>
                            <td style="font-weight:bold;" id="equipmentxi${equilist.eqId}">${equilist.equipmentxi}</td>
                            ...
                            <td>
                                <%--<c:if test="${equilist.type == 1 || equilist.type == '1' }">--%>
                                    <%--<a href="javascript:void(0);" style="color: #00FF00" onclick="police(${equilist.eqId});">正常</a>--%>
                                <%--</c:if>--%>
                                <%--<c:if test="${equilist.type == 2 || equilist.type == '2' }">--%>
                                    <a href="javascript:void(0);" style="" onclick="police(${equilist.eqId});">查看</a>
                                <%--</c:if>--%>
                                <%--<c:if test="${equilist.type == 3 || equilist.type == '3' }">--%>
                                    <%--<a href="javascript:void(0);" style="color: #FFFF33" onclick="police(${equilist.eqId});">异常</a>--%>
                                <%--</c:if>--%>
                            </td>
                            <c:if test="${user.type == 1 || user.type == '1'}">
                                ...
                                <td width="5%">
                                        <%--<c:if test="${captureEnable == null || captureEnable == '' || captureEnable == '0' || captureEnable == 0}">--%>
                                ...
                                <td width="5%">
                                        <%--<c:if test="${captureEnable == null || captureEnable == '' || captureEnable == '0' || captureEnable == 0}">--%>
                                    <a href="javascript:void(0);" onclick="sdtj(${equilist.eqId})">查看</a>
                                        <%--</c:if>--%>
                                </td>
                            </c:if>
                            ...
                            <c:if test="${user.type == 1 || user.type == '1' || user.type == 2 || user.type == '2'}">
                                <td >
                                    <a href="javascript:void(0);" onclick="update(${equilist.eqId},${equilist.substationId})">修改</a>
...
<script type="text/javascript">
    ...
    function goback(p){
        ...
        location.href="${path}/webEquipment/selectEquipment?currentPage="+p+"&type="+type+"&substationId="+substationId+"&substationImage="+substationImage;
    }
    ...
    function delet(eqId){
        ...
        if(confirm("确定删除吗")){
            $.ajax({
                url:'${path}/webEquipment/deletByeqId',
                data:{
                    'eqId' : eqId,
    ...
    function inser(){
        var substationId = $("#substationId").val();
        var substationImage = $("#substationImage").val();
        location.href="${path}/webEquipment/insert?substationId="+substationId+"&substationImage="+substationImage;
    }

    function update(eqId,substationId){
        var substationImage = $("#substationImage").val();
        location.href="${path}/webEquipment/findByeqId?eqId="+eqId+"&substationId="+substationId+"&substationImage="+substationImage;
    }
    ...
    function selectEqui(eqId){
        ...
        if(numberDevices != null && numberDevices != ""){
            location.href="${path}/webTemperatureDetection/realTimeReditect?deviceNumber="+deviceNumber+"&substationImage="+encodeURI(encodeURI(substationImage))+"&substationId="+substationId+"&numberDevices="+numberDevices+"&isRoot="+isRoot+"&deviceSbId="+deviceSbId+"&deviceType="+deviceType;
        }else {
            layer.msg('信息填写不完整无法查看');
        }

    }
    function updateSb(eqId,mute) {
        var index = layer.load(2, {shade: false}); //0代表加载的风格，支持0-2
        var deviceNumber = $('#deviceNumber'+eqId).val();
        if(mute != null && mute != "" && mute != "2" && mute !=2){
            $.ajax({
    ...
    function setEqui(eqId) {
        ...
        if(deviceSbIda != null && deviceSbIda != ''){
            layer.open({
                    ...
                    if (csa == "0" || csa == 0){
                        ...
                        setTimeout(function(){
                            ...
                            if(swId.length==0){
                                ...
                                layer.closeAll('loading');
                                return false;
                            }else if (swId.length < sensorNum || swId.length > sensorNum){
                                $("#csu").val(0);
                                layer.msg('传感器个数不正确')
                                layer.closeAll('loading');
                                return false;
                ...
                '<option value="0">SC14B</option>'+
                '<option value="1">SC15A</option>' +
                '</select>' +
                '</td>' +
                '\t\t\t\t\t\t</tr>\n' +
                '\n' +
                ...
                '\t\t\t\t\t\t<td width="20%" style="font-size: 15px; text-align: center;">\n' +
                '\t\t\t\t\t\t\t采集周期(单位/s)：</td>\n' +
                '\t\t\t\t\t\t<td width="80%"><input type="text" value="10" style="height: 25px;" id="capturePeriod">' +
                '</td>\n' +
                '\t\t\t\t\t</tr>\n' +
                '\t\t\t\t\t<tr height="50px;">\n' +
                '\t\t\t\t\t\t<td width="20%" style="font-size: 15px; text-align: center;">\n' +
    ...
    function sbcjq(eqId) {
        ...
        location.href="${path}/webAcpEquipment/selectByDeverId?deviceNumber="+deviceNumber+"&substationId="+substationId+"&substationImage="+substationImage;
    }
    ...
    function police(eqId) {
        ...
        var substationId = $("#substationId").val();
        var substationImage = $("#substationImage").val();
        location.href="${path}/webAlarmInformation/selectAlarmInformation?deviceNumber="+deviceNumber+"&substationId="+substationId+"&substationImage="+substationImage;
    }
    ...
    function inserAll() {
        ...
        layer.open({
            ...
            ,yes: function(){
                ...
                ajaxFileUploadForType(substationId);
    ...
    function ajaxFileUploadForType(substationId){
        layer.load(1);
        if($('input[type="file"]').val()!=""){
            ...
            if("xls|xlsx".indexOf(extend+"|")==-1){
                ...
                (
                    {
                        url:'${path}/webEquipment/Importsq?substationId='+substationId, //用于文件上传的服务器端请求地址
    ...
    function xz() {
        location.href="${path}/webEquipment/exportsq";
    }
    ...
    function sdtj(eqId) {
         ...
         var equipmentxi = $("#equipmentxi"+eqId).html();
         var deviceNumber = $("#deviceNumber"+eqId).val();
        var substationId = $("#substationId").val();
        var substationImage = $("#substationImage").val();
        location.href="${path}/webPowerOn/selectPowerOn?currentPages="+currentPages+"&deviceNumber="+deviceNumber+"&equipmentxi="+encodeURI(encodeURI(equipmentxi))+"&substationId="+substationId+"&substationImage="+substationImage;
    }
</script>
